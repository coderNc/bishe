    ...
    <div class="proWrapper">
      <div class="proInfo">
        ...
        <div class="headerInfo">
          <h2>{{ actData.title }}</h2>
          <el-button v-if="isAlready" type="success" round class="btn"
            >招募中</el-button
          >
          <el-button v-else type="danger" round class="btn"
            >招募已完成</el-button
          >
          ...
          <el-tabs v-model="activeName" stretch>
            ...
            <el-tab-pane label="项目动态" name="third">
              <div class="footer">
                ...
                <el-progress
                  :percentage="
                    percentage1(actData.fact_num, actData.target_num)
                  "
                  class="progressBar"
                ></el-progress>
              </div>
      ...
      <div class="userInfo">
        ...
        >
          <el-link
            ...
            ><h2 class="orgPlace">{{ actData.place }}</h2></el-link
          >
        </el-tooltip>
...
import { getActivityAndUser, userJoinActivity } from "@/services/activity";
import { getOrgById } from "@/services/org";

export default {
  name: "",
  mixins: [],
  props: {},
  components: { Heder, Xmap },
  data() {
    return {
      isJoin: true,
      activeName: "first",
  ...
  created() {
    ...
    getUserInfo().then(res => {
      ...
      if (res.status == 200 && res.data.data.res_status.code == 10000) {
        var result = res.data.data.activity_info.find(
          ele => ele.activity_id === this.id && ele.status == 0
        );
    ...
    getActivityAndUser({ id: this.id })
      .then(res => {
        ...
        if (res.status == 200 && res.data.data.res_status.code == 10000) {
          ...
          if (
            res.data.data.activity.fact_num == res.data.data.activity.target_num
          ) {
            ...
            .then(() => {
              if (this.isAlready) {
                ...
                userJoinActivity({
                  ...
                  if (
                    res.status == 200 &&
                    res.data.data.res_status.code == 10000
                  ) {
                    this.$message({
                      type: "success",
                      message: "报名成功!"
