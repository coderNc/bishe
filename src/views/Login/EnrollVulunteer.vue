<template>
  <div class="enrollVulunteerWrapper">
    ...
    <div class="tip">
      ...
      <el-button type="danger" class="btn01" @click="EnrollVulunteer('volForm')"
        >申请成为实名注册志愿者</el-button
      >
    </div>
    <div class="step">
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          userRegister({ ...this.volForm }).then(res => {
            console.log(res);
            if (res.status == 200 && res.data.data.res_status.code == 10000) { //todo
              this.$notify({
                title: "成功",
                message: "注册成功！",
                type: "success"
