  ...
  <div>
    <el-table :data="actData" style="width: 100%" border stripe>
      ...
      <el-table-column label="已报名/需求人数" align="center">
        <template slot-scope="scope">{{ scope.row.fact_num }} / {{ scope.row.target_num }}</template>
      </el-table-column>
      ...
      <el-table-column label="活动联系人手机号" align="center">
        <template slot-scope="scope">{{ scope.row.representative_phone }}</template>
      </el-table-column>
      ...
      <el-table-column label="活动持续时间" align="center">
        <template slot-scope="scope">{{ timestampToTime(scope.row.start_time)}} 至 {{ timestampToTime(scope.row.end_time)}}</template>
      </el-table-column>
