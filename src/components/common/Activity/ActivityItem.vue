<template>
  <div class="activityItemWrapper">
    ...
    <div class="header">
      <router-link :to="'/detail/' + activityItem.id">
        <h2 class="titleColor">{{ activityItem.title }}</h2></router-link
      >
      <el-button v-if="!activityItem.already" type="success" round class="btn"
        >招募中</el-button
      >
    ...
    <div class="content">
      ...
      <p>
        项目日期：{{ timestampToTime(activityItem.start_time) }} 至
        {{ timestampToTime(activityItem.end_time) }}
      </p>

      <p>
        报名人数：{{ activityItem.fact_num }} | 目标人数：{{
          activityItem.target_num
        }}
      </p>
    </div>
    <div class="footer">
      <el-progress
        :percentage="
          percentage1(activityItem.fact_num, activityItem.target_num)
        "
        class="progressBar"
      ></el-progress>
    </div>
